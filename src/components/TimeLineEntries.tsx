...
const TimeLineEntries: React.FC = () => {
  const { data } = api.timeline.getEntries.useQuery();

  const deleteItem = (evt: React.MouseEvent<HTMLButtonElement>) => {
    evt.preventDefault();
    const entryId = evt.currentTarget;

    //send a delete request to the server, who is gonna check if it is the admin user to actually delete, then delete
    console.log(entryId);
  };
